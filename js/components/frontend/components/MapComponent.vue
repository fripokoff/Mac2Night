...
export default {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    this.mainMap(); // Appel de mainMap sans localisation
                }
            );
        } else {
    ...
    methods: {
        mainMap: async function (location) {
            ...
            const mapOptions = {
                center: location || { lat: 0, lng: 0 }, // Utiliser une localisation par défaut si non disponible
                zoom: location ? 15 : 2 // Zoom par défaut si localisation non disponible
            };
            const map = new google.maps.Map(this.$refs.theGoogleMap, mapOptions);
            let markers = [];
            if (location) {
