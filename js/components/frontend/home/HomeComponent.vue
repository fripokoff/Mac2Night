    ...
    <section v-if="categories.length > 0" class="mb-12">
        <div class="container">
            <div class="flex items-center justify-between gap-2 mb-6 mt-4">
                <h2 class="_custom-our-menu-title text-2xl font-semibold capitalize">{{ $t("label.our_menu") }}</h2>
                <router-link :to="{ name: 'frontend.menu', query: { s: categoryProps.slug } }"
                    class="_custom-home-view-all-button rounded-3xl capitalize text-sm leading-6 font-medium py-1 px-3 transition text-primary bg-[#0b042b] hover:text-white hover:bg-primary">
                    {{ $t("button.view_all") }}
                </router-link>
            </div>
